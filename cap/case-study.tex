\chapter{Case study: geostatistics on neolithic settlements in Tavoliere}
    ...
    \section{The GIS approach to statistics}
        ...
        \subsection{Bulk distinguishing ditches and compounds}
            ...
            The perimeters calculated and presented in \fref{tab:jnb-results} are not suitable for any other calculation different from classification, since the approach adopted in
            \ldots % cite Angelica 2013
            refers to another derived geometry (this process will be furher explained in \fref{sec:comp-area}).
        ...
        \subsection{Development of the open source webGIS interface}
            Having defined all the algorithms to derive geometries and data from the input vectors, the next step concerned a way to make these tool accessible to the operator, even ones with less experience. To ease the development of a standard compliant interface the Django \emph{framework} has been used (version 1.6); both the framework and the algorithms share the same open source programming language, Python.
            Django enables the development of web-based products; this means that the whole set of functions described in the previous sections is easily accessible using a web browser. The developed web interface consists of few pages (to avoid confusion when navigating); briefly listed:

            \begin{description}
                \item[Shapefiles list] the start page, lists all the uploaded Shapefiles and has a button redirecting to the upload page;
