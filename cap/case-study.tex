\chapter{Case study: geostatistics on neolithic settlements in Tavoliere}
    ...
    \section{The GIS approach to statistics}
        ...
        \subsection{Derive and measure ditches and compounds areas}
            ...
            This method has some drawbacks, mostly the needing to fix the limits of the doorway. A sample result using this approach can be seen in \fref{fig:comp}. Iterating over this method on all the compounds would have generated a consistent error so this method has been discarded.
            ...
            \begin{figure}[H]
                ...
                \hspace{0.1\textwidth}
                \subfloat[Compound's convex hull]{
                    \label{fig:comp-hull}
                    \begin{tikzpicture}[x=1mm,y=1mm,scale=0.2]
                        \input{img/comp-hull} 
                    \end{tikzpicture}
            ...
            . A Python algorithm has been used to automate the process, which saves the derived area geometries to database table separated from the others. Further informations could be derived from these geometries, mostly their perimeter and surface in \si{\meter\squared}, saved as attributes. A sample table with the compound's area and attributes is presented in \fref{tab:tab-area-anglisano}. The derived perimeters and areas are useful for a comparison with the results obtained from a manual deriving of the same data.
